				...
				<tbody style="width:80%; overflow:auto;"> 
					 ...
					 if ( empty($currentResYN) || empty($facilityName)) 
					{
					 ...
					 else 
					 {
							...
							$currentResYN = mysql_real_escape_string($currentResYN);
							$facilityName = mysql_real_escape_string($facilityName);
						
								$query = mysql_query("SELECT ResidentID, FirstName, MiddleInitial, LastName, ResidentType FROM  GenericInfo WHERE ='$currentResYN' AND ='$facilityName';");
