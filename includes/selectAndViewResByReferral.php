			...
			<thead style="width:80%; overflow:auto;" > 
				...
				<tbody style="width:80%; overflow:auto;"> 
						...
						<?php	
							...
							$referralAgency=$_POST['referralAgency'];
							$referralPhoneNum=$_POST['referralPhoneNum'];
							
					//if any fields are empty, redirect back to master search page and do not execute the insert
					 if ( empty($referralName) || empty($referralAgency) || empty($referralPhoneNum) ) 
					{
					 ...
					 else 
					 {
							...
							$referralName = mysql_real_escape_string($referralName);
							$referralAgency = mysql_real_escape_string($referralAgency);
							$referralPhoneNum = mysql_real_escape_string($referralPhoneNum);
						
								$query = mysql_query("SELECT ResidentID, FirstName, MiddleInitial, LastName, ResidentType FROM  WHERE ='$referralName' AND ='$referralAgency' AND ='$referralPhoneNum';");
